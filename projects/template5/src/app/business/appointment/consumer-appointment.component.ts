...
import { QnrDialogComponent } from "jaldee-framework/questionaire/dialog";
...
@Component({
    ...
    styleUrls: ['./consumer-appointment.component.scss']
})
export class ConsumerAppointmentComponent implements OnInit, OnDestroy {
    ...
    getServiceOptions() {
        this.subs.sink = this.consumerService.getServiceoptionsAppt(this.selectedServiceId, this.accountId)
            .subscribe(
                (data) => {
                    if (data) {
    ...
    getServicebyLocationId(locationId, appmtDate) {
        ...
        _this.subs.sink = _this.consumerService.getServicesforAppontmntByLocationId(locationId)
            .subscribe(data => {
                _this.services = data;
    ...
    getAvailableSlotByLocationandService(locid, servid, appmtDate, accountid, type?) {
        ...
        this.subs.sink = _this.consumerService.getSlotsByLocationServiceandDate(locid, servid, appmtDate, accountid)
            .subscribe((data: any) => {
                _this.allSlots = [];
    ...
    getSchedulesbyLocationandServiceIdavailability(locid, servid, accountid) {
        ...
        if (locid && servid && accountid) {
            _this.subs.sink = _this.consumerService.getAvailableDatessByLocationService(locid, servid, accountid)
                .subscribe((data: any) => {
    ...
    actionCompleted() {
        if (this.action !== 'members' && this.action !== 'addmember' && this.action !== 'note' && this.action !== 'attachment') {
            if (this.appointmentType == 'reschedule' && this.scheduledAppointment.service && this.scheduledAppointment.service.priceDynamic) {
                this.subs.sink = this.consumerService.getAppointmentReschedulePricelist(this.scheduledAppointment.service.id).subscribe(
                    (list: any) => {
                        this.priceList = list;
                        let oldprice;
                        let newprice;
    ...
    setRescheduleInfo(uuid) {
        ...
        _this.appmtFor = [];
        return new Promise(function (resolve, reject) {
            _this.subs.sink = _this.consumerService.getAppointmentByConsumerUUID(uuid, _this.accountId).subscribe(
                (appt: any) => {
    ...
    */
    ...
    checkCouponvalidity() {
        ...
        this.subs.sink = this.consumerService.addApptAdvancePayment(param, post_Data).subscribe(data => {
    ...
    addApptAdvancePayment(appmtSlot) {
        ...
        const param = { 'account': this.accountId };
        this.subs.sink = this.consumerService.addApptAdvancePayment(param, post_Data)
            .subscribe(data => {
                this.paymentDetails = data;
                this.paymentLength = Object.keys(this.paymentDetails).length;
                this.jCashInHand = this.paymentDetails.eligibleJcashAmt.jCashAmt;
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (_this.paymentRequestId) {
                ...
                if (_this.selectedService.serviceBookingType === 'request' && (_this.selectedService.date || _this.selectedService.dateTime || _this.selectedService.noDateTime)) {
                    _this.subs.sink = _this.consumerService.postAppointmentRequest(_this.accountId, post_Data)
                        .subscribe(data => {
                            const retData = data;
                    ...
                    _this.subs.sink = _this.consumerService.addCustomerAppointment(_this.accountId, post_Data)
                        .subscribe(data => {
                            const retData = data;
                            _this.appointmentIdsList = [];
                            let parentUid;
    ...
    consumerNoteAndFileSave(uuid) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            _this.subs.sink = _this.consumerService.addAppointmentAttachment(_this.accountId, uuid, dataToSend)
                .subscribe(
                    () => {
                        if (_this.appointmentType !== 'reschedule') {
    ...
    submitQuestionnaire(uuid, paymenttype?) {
        ...
        return new Promise(function (resolve, reject) {
            if (_this.questionnaireList && _this.questionnaireList.labels && _this.questionnaireList.labels.length > 0) {
                ...
                dataToSend.append('question', blobpost_Data);
                _this.subs.sink = _this.questionaireService.submitConsumerApptQuestionnaire(dataToSend, uuid, _this.accountId).subscribe((data: any) => {
                    let postData = {
                        urls: []
                    };
    ...
    */
    ...
    rescheduleAppointment() {
        ...
        this.subs.sink = this.consumerService.rescheduleConsumerApptmnt(this.accountId, post_Data)
            .subscribe(
                () => {
                    this.btnClicked = false;
                    let queryParams = {
    ...
    submitserviceOptionQuestionnaire(uuid) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            dataToSend.append('question', blobpost_Data);
            _this.subs.sink = _this.consumerService.submitConsumerApptServiceOption(dataToSend, uuid, _this.accountId).subscribe((data: any) => {
                resolve(true);
            }, error => {
                _this.isClickedOnce = false;
                let errorObj = _this.errorService.getApiError(error);
    ...
    qnrPopup() {
        const dialogref = this.dialog.open(QnrDialogComponent, {
            width: '50%',
            panelClass: ['popup-class', 'commonpopupmainclass', 'confirmationmainclass', 'serviceoptions'],
            disableClose: true,
            data: {}
        });
