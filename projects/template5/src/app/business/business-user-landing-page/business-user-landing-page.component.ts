...
import { QRCodeGeneratordetailComponent } from 'jaldee-framework/qrcode/generator';
...
@Component({
  selector: 'app-business-user-landing-page',
  templateUrl: './business-user-landing-page.component.html',
  styleUrls: ['./business-user-landing-page.component.css']
})
export class BusinessUserLandingPageComponent implements OnInit, OnDestroy {
  ...
  constructor(    private sharedService: SharedService,
    ...
    this.subscription = this.accountService.getMessage().subscribe(
      (response) => {
  ...
  getLocations() {
    this.consumerService.getLocationsByUserID(this.userId).subscribe (
      (userlocations: any )=> {
        ...
        // if (this.userLocations && this.userLocations.length == 1) {
        //   this.changeLocation(this.userLocations[0])
        // }
      }
    )
  }
  ...
  changeLocation(loc: any) {
    ...
    if (this.activeLocation.id !== this.selectedLocation.id) {
      this.accountService.sendMessage({ ttype: 'changeLocation', data:this.selectedLocation });
  ...
  footerClicked(selectedItem: any) {
    ...
    this.router.navigate([this.customId, 'home'], navigationExtras);
  }
    ...
    _this.authService.goThroughLogin().then(
      (status) => {
        if (status) {
          ...
          let communicateUrl = this.customId + '?callback=communicate';
  ...
  profileActionPerformed(action) {
    if (action === 'coupons') {
      ...
      this.router.navigate([this.customId, this.userId, 'about']);
    }
  }
      ...
      (status) => {
        if (status) {
          if (this.activeCatalog.catalogItem.length == 1 && this.activeCatalog.catalogType == 'submission') {
            ...
            this.router.navigate([this.customId, 'order', 'shoppingcart', 'checkout'], navigationExtras);
          }
          else {
            this.router.navigateByUrl(cartUrl);
          }
  ...
  itemDetails(item) {
    ...
    this.router.navigate([this.customId, 'order', 'item-details'], navigationExtras);
  }
  increment(item) {
    this.addToCart(item);
  }
