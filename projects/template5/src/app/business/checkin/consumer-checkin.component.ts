...
import { ConsumerEmailComponent } from '../consumer-email/consumer-email.component';
...
import { CouponsComponent } from 'jaldee-framework/coupons';
...
import { DomainConfigGenerator } from '../../modules/config/domain/domain-config-generator.service';

@Component({
    selector: 'app-consumer-checkin',
    templateUrl: './consumer-checkin.component.html',
    styleUrls: ['./consumer-checkin.component.scss']
})
    ...
    */
    getServicesbyLocation(locid) {
        ...
        return new Promise(function (resolve, reject) {
            if (locid) {
                _this.subs.sink = _this.consumerService.getServicesByLocationId(locid).subscribe((services) => {
                    resolve(services);
                }, () => {
                    resolve([]);
                })
    ...
    getQueuesbyLocationServiceAndDate(locid, servid, pdate, accountid) {
        const _this = this;
        return new Promise(function (resolve, reject) {
            _this.subs.sink = _this.consumerService.getQueuesbyLocationandServiceId(locid, servid, pdate, accountid)
    ...
    ngOnInit() {
        ...
        this.settingsjson = this.accountService.getJson(this.account['settings']);
    ...
    getServiceOptions() {
        this.subs.sink = this.consumerService.getServiceoptionsWaitlist(this.selectedServiceId, this.accountId)
            .subscribe(
                (data) => {
                    if (data) {
    ...
    getRescheduleWaitlistDet() {
        ...
        _this.waitlist_for = [];
        return new Promise(function (resolve, reject) {
            _this.subs.sink = _this.consumerService.getCheckinByConsumerUUID(_this.rescheduleUserId, _this.accountId).subscribe(
                (waitlst: any) => {
    ...
    rescheduleWaitlist() {
        ...
        this.subs.sink = this.consumerService.rescheduleConsumerWaitlist(this.accountId, post_Data)
            .subscribe(
                () => {
    ...
    getQueuesbyLocationandServiceIdavailability(locid, servid, accountid) {
        ...
        if (locid && servid && accountid) {
            _this.subs.sink = _this.consumerService.getQueuesbyLocationandServiceIdAvailableDates(locid, servid, accountid)
                .subscribe((data: any) => {
    ...
    performCheckin() {
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (_this.paymentRequestId) {
                ...
                _this.subs.sink = _this.consumerService.addCheckin(_this.accountId, post_Data)
                    .subscribe(data => {
                        const retData = data;
                        _this.uuidList = [];
                        let parentUid;
    ...
    submitQuestionnaire(uuid) {
        ...
        return new Promise(function (resolve, reject) {
            if (_this.questionnaireList && _this.questionnaireList.labels && _this.questionnaireList.labels.length > 0) {
                ...
                _this.subs.sink = _this.questionaireService.submitConsumerWaitlistQuestionnaire(dataToSend, uuid, _this.accountId).subscribe((data: any) => {
                    let postData = {
                        urls: []
                    };
    ...
    submitserviceOptionQuestionnaire(uuid) {
        ...
        return new Promise(function (resolve, reject) {  
            ...
            dataToSend.append('question', blobpost_Data);
            _this.subs.sink = _this.consumerService.submitConsumerWaitlistServiceOption(dataToSend, uuid, _this.accountId).subscribe((data: any) => {
    ...
    handleSaveMember() {
        ...
        if (derror === '') {
            ...
            this.subs.sink = fn.subscribe(() => {
                ...
                error => {
                    let errorObj = this.errorService.getApiError(error);
                    this.apiError = this.wordProcessor.getProjectErrorMesssages(errorObj, this.accountService.getTerminologies());
                    this.disable = false;
                });
        } else {
    ...
    getPartysizeDetails(domain, subdomain) {
        this.subs.sink = this.consumerService.getPartysizeDetails(domain, subdomain)
            .subscribe(data => {
    ...
    sendWLAttachment(accountId, uuid, dataToSend) {
        const _this = this;
        return new Promise(function (resolve, reject) {
            _this.subs.sink = _this.consumerService.addWaitlistAttachment(accountId, uuid, dataToSend).subscribe(
                () => {
                    resolve(true);
                }, (error) => {
                    reject(error);
    ...
    getAvailableTimeSlots(QStartTime, QEndTime, interval) {
        ...
        this.selectedSlot = '';
        if (!future) {
            _this.subs.sink = _this.consumerService.getTodayWaitlist(filter).subscribe(
                (waitlist: any) => {
            ...
            _this.subs.sink = _this.consumerService.getFutureWaitlist(filter).subscribe(
                (waitlist: any) => {
                    for (let i = 0; i < waitlist.length; i++) {
                        if (waitlist[i]['appointmentTime']) {
                            activeSlots.push(waitlist[i]['appointmentTime']);
    ...
    checkCouponvalidity() {
        ...
        this.subs.sink = this.consumerService.addWaitlistAdvancePayment(param, post_Data)
            .subscribe(data => {
    ...
    getPic(user) {
        if (user.profilePicture) {
            return this.configService.getJson(user.profilePicture)['url'];
        }
    ...
    updateEmail(post_data) {
        ...
        return new Promise(function (resolve, reject) {
            _this.subs.sink = _this.consumerService.updateProfile(post_data, passtyp)
                .subscribe(
                    () => {
                        resolve(true);
                    },
    ...
    startPayment(paymentMode) {
        ...
        if (_this.isJCashSelected) {
            ...
            this.subs.sink = this.consumerService.consumerPayment(this.paymentReqInfo)
                .subscribe((pData: any) => {
    ...
    getConsumerQuestionnaire() {
        ...
        return new Promise(function (resolve, reject) {
            ...
            _this.subs.sink = _this.consumerService.getConsumerQuestionnaire(_this.selectedServiceId, consumerid, _this.accountId).subscribe(data => {
    ...
    submitOneTimeInfo() {
        ...
        return new Promise(function (resolve, reject) {
            if (_this.onetimeQuestionnaireList && _this.onetimeQuestionnaireList.labels && _this.onetimeQuestionnaireList.labels.length > 0 && _this.onetimeQuestionnaireList.labels[0].questions.length > 0) {
                ...
                dataToSend.append('question', blobpost_Data);
                _this.subs.sink = _this.consumerService.submitCustomerOnetimeInfo(dataToSend, _this.providerConsumerId, _this.accountId).subscribe((data: any) => {
                    resolve(true);
                },
                    error => {
                        _this.isClickedOnce = false;
