...
import { DateTimeProcessor, ServiceMeta } from "jconsumer-shared";
import { TeleBooking } from "./shared/models/tele-booking-model";
...
export class TeleBookingService {
    ...
    async getTeleBookingFromCheckIn(uid, source, accountId?) {
        try {
            const bookingObj: any = await firstValueFrom(this.getTeleCheckinById(uid, source, accountId));
            let token = '';
    ...
    async getTeleBookingFromAppt(uid: string, source: string, accountId?: string): Promise<TeleBooking> {
        try {
            const bookingObj: any = await firstValueFrom(this.getTeleApptById(uid, source, accountId));
            let busName = "";
            let busId = ""
            let userName;
            let userId;
    ...
    getCatalogItemsByPrice(accountId, storeEncId, from, count, minPrice, maxPrice, parentItemId = 0, filters: any = {}, sort: any = {}) {
        const params: string[] = [
            `accountId-eq=${accountId}`,
            `storeEncId-eq=${storeEncId}`,
            `from=${from}`,
            `count=${count}`,
            `parentItemId-eq=${parentItemId}`,
            `price-gt=${minPrice}`,
            `price-lt=${maxPrice}`
        ];
        ...
        const url = `consumer/so/catalog/item?${params.join('&')}`;
        return this.servicemeta.httpGet(url);
    }

    getCatalogItemsWithSort(accountId, storeEncId, from, count, parentItemId = 0, filters: any = {}, sort: any = {}) {
        const params: string[] = [
            `accountId-eq=${accountId}`,
            `storeEncId-eq=${storeEncId}`,
            `from=${from}`,
