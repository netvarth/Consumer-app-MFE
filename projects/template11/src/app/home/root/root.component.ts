...
import { AccountService, AuthService, BookingService, ConsumerService, DateTimeProcessor, GroupStorageService, JGalleryService, LocalStorageService, Messages, OrderService, projectConstantsLocal, SharedService, SubscriptionService, ToastService, WordProcessor } from 'jconsumer-shared';
...
export class RootComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  actionPerformed(actionObj) {
    ...
    } else if(actionObj['type'] === 'order') {
      if(actionObj['action']==='view') {

      } else {
        this.reOrder(actionObj['value']);
      }
    } else {
      const navigationExtras: NavigationExtras = {
        queryParams: {
...
showOrderDetails(order) {
    ...
    this.router.navigate([this.sharedService.getRouteID(), 'orderdetails'], navigationExtras);
  ...
  reOrder(order) {
    ...
    this.orderService.getOrderByUid(order.uid).subscribe(orderData => {
      ...
      this.reCreateCart(orderData).then(data => {
  ...
  getCategories() {
    this.orderService.getItemCategory(this.accountId).subscribe(
      (categories: any) => {
        ...
        this.isUrlsLoaded = true;
        console.log("this.itemCategories", this.itemCategories);
      }
    )
  }
}
