...
- Store hero search reuses the items page's `app-item-search` with the active account and `extras.selectedCatalogs`. Typing shows product suggestions, selecting a product opens its detail page, and Enter or the search button opens the items page with the trimmed query. Placeholder, accessible label and position still come from the hero search configuration.
- Hero images preserve their complete proportions. Store search uses bounded percentage coordinates. The entire service hero links to `service.hero.action.link` when the action is enabled and valid; legacy `action.hotspot` coordinates no longer limit its clickable area. Disabling the action cannot remove a button painted into the artwork.
- The service section cover is decorative and does not navigate. Existing `services.coverImage.link` values are ignored; individual service cards retain their configured links.
