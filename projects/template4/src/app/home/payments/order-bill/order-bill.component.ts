...
import { AccountService, ConsumerService, DateFormatPipe, ErrorMessagingService, Messages, OrderService, PaytmService, projectConstantsLocal, RazorpayService, SharedService, ToastService, WordProcessor } from 'jconsumer-shared';
...
export class OrderBillComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        let subs = this.activated_route.queryParams.subscribe(
            params => {
                console.log(params);
        ...
        if (ynwUuid) {
            this.orderService.getInvoiceByOrderYnwuuid(accId, invoiceId)
                .subscribe(data => {
                    ...
                    if (this.invoiceDetailsById && this.invoiceDetailsById.store && this.invoiceDetailsById.store.details && this.invoiceDetailsById.store.details.uploadedDocuments && this.invoiceDetailsById.store.details.uploadedDocuments.length > 0 && this.invoiceDetailsById.store.details.uploadedDocuments[0].s3path) {
                        this.storeLogo = this.invoiceDetailsById.store.details.uploadedDocuments[0].s3path;
                    }
    ...
    getPrePaymentDetails() {
        ...
        let subs = this.consumerService.getPaymentDetail(params, this.uuid)
            .subscribe(
                data => {
        ...
        if (this.jcashamount > 0 && this.checkJcash) {
                        ...
                        this.consumerService.PayByJaldeewallet(postData)
                            .subscribe(data => {
                                ...
                                if (!this.wallet.isGateWayPaymentNeeded && this.wallet.isJCashPaymentSucess) {
                                    this.toastService.showError(Messages.PROVIDER_BILL_PAYMENT)
            ...
            if (this.selected_payment_mode && this.selected_payment_mode.toLowerCase() === 'cash') {
                ...
                if (this.pay_data.uuid != null &&
                    ...
                    let subs = this.consumerService.consumerPayment_invocie(this.pay_data)
                        .subscribe(
                            (data: any) => {
                                this.origin = 'consumer';
    ...
    finishTransaction(status) {
        if (status) {
            ...
            if (this.checkIn_type === 'order') {
                this.ngZone.run(() => this.router.navigate([this.sharedService.getRouteID(), 'history'], { queryParams: { 'is_orderShow': 'true' } }));
            } else {
                this.ngZone.run(() => this.router.navigate([this.sharedService.getRouteID(), 'dashboard'], navigationExtras));
            }
        } else {
            this.isClickedOnce = false;
            this.loadingPaytm = false;
            ...
            if (this.checkIn_type === 'order') {
                ...
                this.ngZone.run(() => this.router.navigate([this.sharedService.getRouteID(), 'order', 'bill'], navigationExtras));
            } else {
                const navigationExtras: NavigationExtras = {
                    queryParams: {
                        uuid: this.uuid,
            ...
            let subs = this.consumerService.consumerPayment_invocie(this.pay_data)
                .subscribe(
                    data => {
                        ...
                        this.toastService.showError(this.wordProcessor.getProjectMesssages('CHECKIN_SUCC_REDIRECT'))
                        setTimeout(() => {
   ...
   */
        ...
        return new Promise<void>((resolve, reject) => {
            let subs = this.consumerService.applyCoupon(action, uuid, this.accountId).subscribe
                (billInfo => {
                    ...
                    error => {
                        let errorObj = this.errorService.getApiError(error);
                        this.toastService.showError(errorObj); reject(error);
                    });
            this.subscriptions.add(subs)
        });
    }
    ...
    printMe() {
        ...
        bill_html += '	<tr>';
        bill_html += '<td width="40%" style="text-align:left;">' + 'Item';
        bill_html += '</td>';
        bill_html += '<td width="20%" style="text-align:left;">' + 'Rate(₹)';
        bill_html += '</td>';
        bill_html += '<td width="20%" style="text-align:right">' + 'Quantity' + '</td>';
        bill_html += '<td width="20%" style="text-align:right">' + 'Price(₹)' + '</td>';
        bill_html += '	</tr>';
        bill_html += '</table>';
        bill_html += '	</td></tr>';
        ...
        for (const item of this.invoiceDetailsById.items) {
            ...
            bill_html += '	style="color:#000000; font-size:10pt; line-height:15px; font-family:Ubuntu, Arial,sans-serif; padding-top:5px;padding-bottom:5px">';
            bill_html += '	<tr>';
            bill_html += '<td width="40%" style="text-align:left;">' + item.orderItemDetails.spItem.name;
            bill_html += '<td width="20%" style="text-align:left;">' + parseFloat(item.price).toFixed(2);
            if (item.GSTpercentage > 0) {
                bill_html += '<span style="font-size: .60rem;;font-weight: 600;color: #fda60d;"><sup> Tax</sup></span>';
            }
            bill_html += '</td>';
            bill_html += '<td width="20%" style="text-align:right">' + item.quantity + '</td>';
            bill_html += '<td width="20%" style="text-align:right">' + (item.quantity * item.price).toFixed(2) + '</td>';
            bill_html += '	</tr>';
            if (item.discount) {
        ...
        if (this.invoiceDetailsById.taxTotal !== 0) {
            ...
            bill_html += '<table width="100%" style="color:#000000; font-size:10pt; font-family:Ubuntu, Arial,sans-serif; ;padding-bottom:5px">';
            bill_html += '	<tr>';
            bill_html += '<td width="70%" style="text-align:right">Tax Total</td>';
        ...
        if (this.invoiceDetailsById.netTotalWithTax !== 0 && this.invoiceDetailsById.taxTotal !== 0) {
            ...
            bill_html += '<table width="100%" style="color:#000000; font-size:10pt; font-family:Ubuntu, Arial,sans-serif; ;padding-bottom:5px">';
            bill_html += '	<tr>';
            bill_html += '<td width="70%" style="text-align:right">Net Total With Tax</td>';
     ...
     */
    cashPayment() {
        this.toastService.showError(`Visit ${this.provider_label} to ${this.selected_payment_mode}
        `)
        setTimeout(() => {
